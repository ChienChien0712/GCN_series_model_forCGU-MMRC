...
# Hyper-parameters
dataset = sys.argv[1] #fold url
n_folds = int(sys.argv[2])
training_size_p = float(sys.argv[3]) # 0~1
...
model_name = sys.argv[9] #'Graph-U-Net' 
filters_gcn = [int(i.strip()) for i in sys.argv[10].split(',')]
graph_kernel = sys.argv[11] #'Normalized-Laplacian' #Normalization
gcn_activation = eval('nn.'+sys.argv[12]+'(inplace=True)') #'ELU' #'ReLU'
dropout_gcn = float(sys.argv[13])
gcn_bias = eval(sys.argv[14])
#gPool
pooling_ratios = [float(i.strip()) for i in sys.argv[15].split(',')]
topk_activation =  sys.argv[16] #'sigmoid' 'tanh'
#gUnpool
gUnpool = sys.argv[17] #'gUnpool+residual_connection' #'gUnpool' 'None'
if gUnpool != 'None':
    gUnpool_gcn = filters_gcn[:-1]
    gUnpool_gcn.reverse()
#information pooling
pooling_method = sys.argv[18] #max,sum,mean
pooling_by_layer_concatenation = eval(sys.argv[19]) #True #False
#fc
n_hidden_fc = int(sys.argv[20]) #0 or int
fc_activation = eval('nn.'+sys.argv[21]+'(inplace=True)')#'ELU' #'ReLU'
dropout_fc = float(sys.argv[22])
fc_bias = eval(sys.argv[23])

#device
device = sys.argv[24]  # 'cuda', 'cpu'
seed = sys.argv[25]
threads = int(sys.argv[26]) #線程數目
log_interval = int(sys.argv[27])

#output folder
output_folder = sys.argv[28]
...
class GraphUnet(nn.Module):
    def __init__(self,
        ...
        if pooling_by_layer_concatenation == True:
            if gUnpool != 'None':
                in_fc = sum(gUnpool_gcn)*pooling_noumber
            elif gUnpool == 'None':
                in_fc = sum(filters_gcn)*pooling_noumber
        elif pooling_by_layer_concatenation == False: 
            if gUnpool != 'None':
    ...
    def forward(self, data):
        ...
        for layer, gconv in enumerate(self.gconv):
            ...
            A_dict[layer] = W
            x_dict[layer] = x
            N_nodes_dict[layer] = torch.sum(mask, dim=1).reshape(len(torch.sum(mask, dim=1)),1)
            #print(N_nodes_dict[0])
            # gPool
